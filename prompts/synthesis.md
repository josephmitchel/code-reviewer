...
- **Concern bodies stand alone:** only user-facing questions appear in the posted report; non-user-facing ones are decided silently and never shown. So a concern body must never point at its question ("see the attached question", "per the question below") — if the decision matters to the reader, state the recommended direction inline in the body instead.
